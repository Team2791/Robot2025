...
public class SlewWrapper {
    public record SlewOutputs(double xspeed, double yspeed, double rot) { }
    ...
    public SlewOutputs update(double xspeed, double yspeed, double rot) {
        ...
        double diff = SwerveUtil.angleDifference(theta, this.prevDir);

        if (diff < Math.PI * 0.45) {
            this.prevDir = SwerveUtil.stepTowardsAngular(this.prevDir, theta, directionSlew * dt);
            this.prevMag = this.slewMag.calculate(mag);
